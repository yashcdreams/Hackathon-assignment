...
const RoleListComponent = () => {
  ...
  const fetchRoles = useCallback(async () => {
    ...
    const formattedRows = fetchedRoles?.map((role, index) => ({
      id: index + 1,
      name: role.roleName,
      status: <span style={{ color: role.status ? "green" : "red" }}>{role.status ? "Active" : "Inactive"}</span>,
      actions: (
        <td>
          <img
            src="./edit icon.svg"
  ...
  return (
    <>
      <Tab.Container>
        <Tab.Content>
          <Container fluid className="d-flex justify-content-between align-items-center mt-3">
            <h2>
